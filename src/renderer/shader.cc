...
void TransformShader::vertex(VertexInData& _in, VertexOutData& _out) {
    ...
    out.color = in.color;
    gl_Position = in.projectionMatrix * in.viewMatrix * in.modelMatrix *
                  glm::vec4(in.position, 1.0f);
    // spdlog::debug("x,y,z {} {}
    // {}",in.modelMatrix[3][0],in.modelMatrix[3][1],in.modelMatrix[3][2]);
    return;
}
...
void SSAOShader::vertex(VertexInData& _in, VertexOutData& _out) {
    ...
    gl_Position = in.projectionMatrix * in.viewMatrix * in.modelMatrix *
                  glm::vec4(in.position, 1.0f);
    auto worldPos =
        in.modelMatrix * glm::vec4(in.position, 1.0f); // vec3 = vec4...
...
void GouraudShader::vertex(VertexInData& _in, VertexOutData& _out) {
    ...
    gl_Position = in.projectionMatrix * in.viewMatrix * in.modelMatrix *
                  glm::vec4(in.position, 1.0f); // mvp transformation
    ...
    for (const auto& light : in.directionLights) {
        ...
        glm::vec3 viewDir =
            glm::normalize(in.camPos - in.position);         // pos -> view
        glm::vec3 half = glm::normalize(lightDir + viewDir); // 半程向量
        glm::vec3 L_a = light->_ambient * kd;
        glm::vec3 L_d = light->_diffuse * kd *
                        std::max(0.0f, glm::dot(in.normal, lightDir));
        glm::vec3 L_s = light->_specular * ks *
                        pow(std::max(0.0f, glm::dot(in.normal, half)),
    ...
    for (const auto& light : in.pointLights) {
        glm::vec3 lightDir =
            glm::normalize(light->_position - in.position); // pos -> light
        glm::vec3 viewDir =
            glm::normalize(in.camPos - in.position);         // pos -> view
        glm::vec3 half = glm::normalize(lightDir + viewDir); // 半程向量
    ...
    // color = glm::vec3(std::min(color[0],255.0f),
    // std::min(color[1],255.0f),std::min(color[2],255.0f));
    out.color = glm::vec4(
        glm::clamp(color, glm::vec3(0, 0, 0), glm::vec3(255, 255, 255)), 1.0f);
...
void PhongShader::fragment(FragmentInData& _in, FragmentOutData& _out) {
    ...
    for (const auto& light : in.directionLights) {
        glm::vec3 lightDir = glm::normalize(-light->_direction); // pos -> light
        glm::vec3 viewDir =
            glm::normalize(in.camPos - in.worldPos);         // pos -> view
        glm::vec3 half = glm::normalize(lightDir + viewDir); // 半程向量
    ...
    for (const auto& light : in.pointLights) {
        glm::vec3 lightDir =
            glm::normalize(light->_position - in.worldPos); // pos -> light
        glm::vec3 viewDir =
            glm::normalize(in.camPos - in.worldPos);         // pos -> view
        glm::vec3 half = glm::normalize(lightDir + viewDir); // 半程向量
        ...
        glm::vec3 L_s = ks * std::pow(std::max(0.0f, glm::dot(in.normal, half)),
                                      in.material->shininess);
    ...
    gl_FragColor = glm::vec4(glm::clamp(color, glm::vec3(0.0f, 0.0f, 0.0f),
